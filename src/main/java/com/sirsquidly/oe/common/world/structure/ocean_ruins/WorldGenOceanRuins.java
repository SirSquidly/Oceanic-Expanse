...
public class WorldGenOceanRuins extends WorldGenerator
{
    ...
    private static final String[] BigRuins = new String[] {"underwater_ruin_bigbrick1", "underwater_ruin_bigbrick2"};

    private static final String[] SmallRuins = new String[] {"underwater_ruin_brick1", "underwater_ruin_brick2", "underwater_ruin_brick3",
            "underwater_ruin_brick4", "underwater_ruin_brick5", "underwater_ruin_brick6", "underwater_ruin_newbrick1",
            "underwater_ruin_newbrick2", "underwater_ruin_newbrick3"};
    ...
    public static boolean isAbleToSpawnHere(BlockPos pos, World world)
    {
        ...
        if(BiomeDictionary.hasType(biomeCurrently, BiomeDictionary.Type.OCEAN))
        {
            ...
            {
                if(BiomeDictionary.hasType(biomeCurrently, types)) return false;
            }
            return true;
        }
        return false;
    ...
    public static class Start extends StructureStart
    {
        ...
        private void create(World worldIn, Random rnd, int chunkX, int chunkZ)
        {
            ...
            /* The 30% chance a Ruin is a Large One. */
            boolean genLargeRuin = random.nextDouble() < 0.3D;

            /* DON'T ADD +8 OFFSET, Big Ruins take the whole chunk as is. */
            BlockPos posI = new BlockPos(chunkX * 16, 0, chunkZ * 16);
            ...
            if(y != 0)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (!genLargeRuin) blockpos = blockpos.add(8,0,8);

                    OceanRuins structure = new OceanRuins(worldIn, worldIn.getSaveHandler().getStructureTemplateManager(), components);

                    structure.startBuilding(blockpos, rotation, genLargeRuin ? BigRuins[random.nextInt(BigRuins.length)] : SmallRuins[random.nextInt(SmallRuins.length)]);
                    ...
                    {
                        if (genLargeRuin) generateSurroundingRuins(worldIn, random, chunkX, chunkZ);
                        break;
                    }
                }
            }
        ...
        private void generateSurroundingRuins(World world, Random rand, int centerChunkX, int centerChunkZ)
        {
            ...
            int ruinsToPlace = 4 + rand.nextInt(5);

            for (int i = 0; i < ruinsToPlace; i++)
            {
                BlockPos off = offsets.get(i);


                int chunkX = centerChunkX + off.getX();
                int chunkZ = centerChunkZ + off.getZ();

                int baseX = chunkX << 4;
                int baseZ = chunkZ << 4;

                int margin = 4;
                int localX = margin + rand.nextInt(16 - margin * 2);
                int localZ = margin + rand.nextInt(16 - margin * 2);

                BlockPos posI = new BlockPos(baseX + localX, 0, baseZ + localZ);

                int y = getSurfaceHeight(world, posI, ConfigHandler.worldGen.oceanRuins.ruins_min_y, ConfigHandler.worldGen.oceanRuins.ruins_max_y);
                if (y == 0) continue;

                BlockPos spawnPos = posI.up(y);

                Rotation rotation = Rotation.values()[rand.nextInt(Rotation.values().length)];
                String ruin = SmallRuins[rand.nextInt(SmallRuins.length)];


                OceanRuinsTemplate template = new OceanRuinsTemplate( world.getSaveHandler().getStructureTemplateManager(), ruin, spawnPos, rotation, 0, true);
